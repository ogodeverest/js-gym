...
let labels = [];
repeat(6, (i) => labels.push(`Unit ${i + 1}`));
...
function map(array, transform) {
  let transformed = [];
  for (let entry of array) {
    transformed.push(transform(entry));
  }
...
//Script with the most characters
...
console.log(
  SCRIPTS.reduce((a, b) => (charachterCount(a) > charachterCount(b) ? a : b))
);
...
function loop(start, test, update, fn) {
...
function every(array, test) {
  return !array.some((entry) => !test(entry));
}
...
//Dominant writing direction - SCRIPTS

function dominantDirection(text) {
  const dominant = countBy(text, (char) => {
